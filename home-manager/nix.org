...
** Overlay
#+begin_src nix-ts :tangle flake/overlay.nix
{ inputs, self, ... }:
...
** Emacs Package Instantiation
#+begin_src nix-ts :tangle flake/epkgs.nix
{ inputs, ... }:
...
#+begin_src nix-ts :tangle scripts/mpvmacs.nix
{ inputs, ... }:
...
*** HDMI Helper
#+begin_src nix-ts :tangle scripts/hdmi-helper.nix
{ inputs, ... }:
