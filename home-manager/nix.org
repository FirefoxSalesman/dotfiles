...
* Packages
** Overlay
#+begin_src nix-ts :tangle modules/packages/overlay.nix
{ inputs, self, ... }:
...
** Scripts
*** HDMI Helper
#+begin_src nix-ts :tangle modules/packages/scripts/hdmi-helper.nix
{ inputs, ... }:
...
#+begin_src nix-ts :noweb yes :tangle modules/packages/scripts/pkg.nix
{ inputs, ... }:
{
  perSystem =
    {
...
+ When you set up a machine, add this desktop entry
#+begin_src
[Desktop Entry]
Name=Org-Protocol
Exec=emacsclient %u
...
* Home Manager
** Main Config
#+begin_src nix-ts :tangle modules/home.nix
{ self, inputs, ... }:
let
  system = "x86_64-linux";
...
** Host-Predicated Code
#+begin_src nix-ts :tangle modules/hosts.nix
{
  flake.homeModules = {
    emacs-host.hosts = {
      xrandr-command = ''
...
** User Dirs
#+begin_src nix-ts :tangle modules/user-dirs.nix
{
...
*** aliases
#+NAME: Shell Aliases
#+begin_src nix-ts :tangle modules/shells/aliases.nix
{
...
#+begin_src nix-ts :tangle modules/shells/fish.nix
{
  flake.homeModules.shellConfig =
    { lib, pkgs, ... }:
    {
...
#+begin_src nix-ts :tangle modules/shells/bash.nix
{
  flake.homeModules.shellConfig =
    { pkgs, lib, ... }:
    {
...
*** eshell
#+begin_src nix-ts :tangle modules/shells/eshell.nix :noweb yes
{
  flake.homeModules.shellConfig =
    {
      pkgs,
...
#+begin_src nix-ts :tangle modules/shells/starship.nix
{
...
*** dash
Probably won't use this one much interactively, but it's my login shell, & my /bin/sh, so it may as well be nice
#+begin_src nix-ts :tangle modules/shells/dash.nix :noweb yes
{
  flake.homeModules.shellConfig =
    { pkgs, lib, ... }:
    {
...
***** Premble
#+begin_src nix-ts :tangle modules/shells/dash-template.nix :noweb yes
{
...
#+begin_src nix-ts :tangle modules/shells/environment-variables.nix
{
  flake.homeModules.shellConfig =
    { lib, config, ... }:
    {
...
** Aesthetics
*** Icons
#+begin_src nix-ts :tangle modules/aesthetics/icons.nix :noweb yes
{ inputs, ... }:
...
*** Transparency
At the moment we're using picom
#+begin_src nix-ts :tangle modules/aesthetics/transparency.nix
{
...
#+begin_src nix-ts :tangle modules/aesthetics/colors.nix :noweb yes
{ inputs, ... }:
...
*** Modeline
**** i3bar
We use this to format the date, time, & battery percentage in our modeline.
#+begin_src nix-ts :tangle modules/aesthetics/modeline/i3bar.nix
{
  perSystem = { pkgs, ... }: {
...
#+begin_src nix-ts :tangle modules/aesthetics/modeline/nano.nix :noweb yes
{ inputs, ... }:
...
**** Tab Bar
#+begin_src nix-ts :tangle modules/aesthetics/modeline/tab.nix
{
...
#+begin_src nix-ts :tangle modules/aesthetics/posframe.nix
{
  flake.homeModules.aesthetics.programs.emacs.init = {
...
#+begin_src nix-ts :tangle modules/aesthetics/shrface.nix :noweb yes
{
...
#+begin_src nix-ts :tangle modules/aesthetics/writeroom.nix
{
...
*** Qutebrowser
#+begin_src nix-ts :tangle modules/aesthetics/qutebrowser.nix
{
...
*** Mustache File
#+begin_src json-ts :tangle modules/aesthetics/pywal.json.mustache
{
  "special": {
...
** Extra Packages
#+begin_src nix-ts :tangle modules/extra-packages.nix
{
...
*** Misc
#+begin_src nix-ts :tangle modules/development/misc.nix
{
...
*** Project Management
#+begin_src nix-ts :tangle modules/development/project-management.nix :noweb yes
{
  flake.homeModules.development =
    { pkgs, lib, ... }:
    {
...
*** Web
#+begin_src nix-ts :tangle modules/development/web.nix :noweb yes
{
...
*** Nix
#+begin_src nix-ts :tangle modules/development/nix.nix
{
...
*** Elisp
#+begin_src nix-ts :tangle modules/development/elisp.nix
{
...
*** Java
#+begin_src nix-ts :tangle modules/development/java.nix :noweb yes
{
...
*** C
#+begin_src nix-ts :tangle modules/development/c.nix
{
...
*** Python
#+begin_src nix-ts :tangle modules/development/python.nix
{
...
#+begin_src nix-ts :tangle modules/development/eglot.nix :noweb yes
{
  flake.homeModules.development =
    {
      pkgs,
...
*** Ledger
#+begin_src nix-ts :tangle modules/development/ledger.nix
{
...
#+begin_src nix-ts :tangle modules/file-management.nix :noweb yes
{ inputs, ... }:
{
  perSystem =
    { pkgs, ... }:
...
** Emacs
*** Main
#+begin_src nix-ts :tangle modules/emacs.nix :noweb yes
{ inputs, ... }:
...
#+begin_src nix-ts :tangle modules/emacs/ui.nix
{
...
+ C-x C-q: Exit wgrep
#+begin_src nix-ts :tangle modules/emacs/wgrep.nix
{
...
*** Early Init
#+begin_src nix-ts :tangle modules/emacs/early-init.nix :noweb yes
{
...
#+begin_src nix-ts :tangle modules/emacs/evil.nix :noweb yes
{ inputs, ... }:
...
**** Extensions
***** Surround
#+begin_src nix-ts :tangle modules/emacs/evil/surround.nix
{
  flake.homeModules.emacs.programs.emacs.init = {
...
***** Easymotion
#+begin_src nix-ts :tangle modules/emacs/evil/easymotion.nix :noweb yes
{
  flake.homeModules.emacs.programs.emacs.init = {
    keybinds.avy = {
      enable = true;
...
(defun avy-action-cursor (pt)
  "Make a cursor at point."
  (nix-emacs-base-avy-action 'evil-mc-make-cursor-here pt))
...
***** Multi Cursor
Maybe try lasgun? https://github.com/aatmunbaxi/lasgun.el/blob/master/lasgun.el
Cursors hydra: https://hungyi.net/posts/hydra-for-evil-mc/
#+begin_src nix-ts :tangle modules/emacs/evil/multicursor.nix :noweb yes
{
...
***** Exchange
#+begin_src nix-ts :tangle modules/emacs/evil/exchange.nix
{
  flake.homeModules.emacs = {
...
**** Leader Key
Note: You still need to install udisks manually if you're not using this on nixos.
#+begin_src nix-ts :tangle modules/emacs/evil/leader.nix
{ inputs, ... }:
...
#+begin_src nix-ts :tangle modules/emacs/evil/ergovim.nix :noweb yes
{
...
#+begin_src nix-ts :tangle modules/emacs/evil/symex.nix :noweb yes
{
  flake.homeModules.emacs.programs.emacs.init = {
    ide.symex = true;
    usePackage.symex = {
...
**** Compats
#+begin_src nix-ts :tangle modules/emacs/evil/compats.nix :noweb yes
{
...
*** Completion System
My completion system is based around vertico & corfu, because that's what all the cool kids use nowadays.

**** Vertico
Vertico is basically dmenu for emacs. It lists out all the options in a command menu.
#+begin_src nix-ts :tangle modules/emacs/completions/vertico.nix :noweb yes
{
  flake.homeModules.emacs.programs.emacs.init = {
    completions = {
      vertico.enable = true;
...
format count borrowed from: https://github.com/rougier/nano-vertico
#+NAME: Emacs Vertico Config
#+begin_src emacs-lisp
...
#+begin_src nix-ts :tangle modules/emacs/completions/corfu.nix :noweb yes
{
  flake.homeModules.emacs.programs.emacs.init = {
    completions = {
      tempel.enable = true;
...
#+begin_src nix-ts :tangle modules/emacs/completions/consult.nix :noweb yes
{
  flake.homeModules.emacs.programs.emacs.init = {
...
The version of embark in nixpkgs doesn't work right with org mode because it's missing some inputs

Credits:
Which-key indicator from the embark wiki
avy actions from karthink
#+begin_src nix-ts :tangle modules/emacs/completions/embark.nix :noweb yes
{ inputs, ... }:
...
**** App Launcher
#+begin_src nix-ts :tangle modules/emacs/completions/app-launcher.nix
{ inputs, ... }:
...
**** Search
#+begin_src nix-ts :tangle modules/emacs/completions/search.nix
{
  flake.homeModules.emacs = { pkgs, ... }: {
...
#+begin_src nix-ts :tangle modules/emacs/completions/ezf.nix :noweb yes
{ inputs, ... }:
{
  perSystem =
    { pkgs, self', ... }:
...
#+begin_src nix-ts :tangle modules/emacs/help-system.nix :noweb yes
{ inputs, ... }:
...
*** Repeaters
All of this config was origally stolen from Karthinks. I don't think any of the code I stole is actually here anymore, though.
#+begin_src nix-ts :tangle modules/emacs/repeaters.nix :noweb yes
{ inputs, ... }:
{
  perSystem =
    { pkgs, ... }:
...
*** Hydra
#+begin_src nix-ts :tangle modules/emacs/hydra.nix :noweb yes
{
  flake.homeModules.emacs.programs.emacs = {
...
*** Proced
#+begin_src nix-ts :tangle modules/proced.nix
{
...
#+begin_src nix-ts :tangle modules/notifications.nix :noweb yes
{
...
#+begin_src nix-ts :tangle modules/writing/org.nix :noweb yes
{ inputs, ... }:
{
...
#+begin_src nix-ts :tangle modules/writing/latex.nix
{
  flake.homeModules.writing =
    { pkgs, ... }:
    {
...
*** Markdown
#+begin_src nix-ts :tangle modules/writing/markdown.nix :noweb yes
{
...
*** Tools
#+begin_src nix-ts :tangle modules/writing/tools.nix
{
  flake.homeModules.writing =
    { pkgs, ... }:
    {
...
#+begin_src nix-ts :tangle modules/passwords/pinentry.nix
{
...
#+NAME: Emacs Pass
#+begin_src nix-ts :tangle modules/passwords/pass.nix
{
  flake.homeModules.passwordManagement =
    { pkgs, ... }:
    {
...
#+begin_src nix-ts :noweb yes :tangle modules/window-manager/tab-bar.nix
{ inputs, ... }:
...
*** Javelin
#+begin_src nix-ts :tangle modules/window-manager/javelin.nix
{
...
#+begin_src nix-ts :tangle modules/window-manager/bufler.nix :noweb yes
{
  flake.homeModules.wm.programs.emacs.init.usePackage = {
    bufler = {
      enable = true;
...
*** ELWM
#+begin_src nix-ts :tangle modules/window-manager/elwm.nix :noweb yes
{
...
*** EXWM
#+begin_src nix-ts :tangle modules/window-manager/exwm.nix :noweb yes
{
  flake.homeModules.exwm =
    {
      config,
...
*** EWM
#+begin_src nix-ts :tangle modules/window-manager/ewm.nix
{
  perSystem = { lib, pkgs, ... }: {
...
*** Popper
#+begin_src nix-ts :tangle modules/window-manager/popper.nix
{
...
*** Ace Window
#+begin_src nix-ts :tangle modules/window-manager/ace.nix :noweb yes
{
...
** Gptel
https://sukany.cz/blog/2026-03-23-emacs-gptel-setup/

#+begin_src nix-ts :tangle modules/gptel.nix :noweb yes
{ inputs, ... }:
...
** Multimedia
*** AV Editing
#+begin_src nix-ts :tangle modules/multimedia/av.nix
{ inputs, ... }:
...
*** MPD
#+begin_src nix-ts :tangle modules/multimedia/mpd.nix :noweb yes
{ inputs, ... }:
...
#+begin_src nix-ts :tangle modules/multimedia/mpv.nix :noweb yes
{ inputs, ... }:
...
*** Pulseaudio Control
#+begin_src nix-ts :tangle modules/multimedia/pavucontrol.nix
{
  flake.homeModules.media =
    { pkgs, ... }:
    {
...
#+begin_src nix-ts :tangle modules/multimedia/podcast.nix :noweb yes
{
  perSystem =
    { pkgs, self', ... }:
    let
...
#+begin_src nix-ts :tangle modules/sage.nix :noweb yes
{
...
*Important*: Add your user to the input & uinput groups
#+begin_src nix-ts :tangle modules/keyboard.nix :noweb yes
{
...
#+begin_src nix-ts :tangle modules/web/browseurl.nix
{
...
*** EWW
It's a simple text based web browser. I use it for short web searches
#+begin_src nix-ts :tangle modules/web/eww.nix :noweb yes
{
...
*** Yeetube
#+begin_src nix-ts :tangle modules/web/yeetube.nix
{
...
*** Webjump
#+begin_src nix-ts :tangle modules/web/webjump.nix :noweb yes
{
...
*** Elfeed
#+begin_src nix-ts :tangle modules/web/elfeed.nix
{
...
#+begin_src nix-ts :tangle modules/web/qutebrowser/misc.nix :noweb yes
{
...
#+begin_src nix-ts :tangle modules/web/qutebrowser/keybinds.nix
{
...
#+begin_src nix-ts :tangle modules/web/qutebrowser/greasemonkey.nix
{
  flake.homeModules.web = { pkgs, ... }:
...
#+begin_src nix-ts :tangle modules/web/qutebrowser/privacy.nix
{
...
**** Emacs Integration
#+begin_src nix-ts :tangle modules/web/qutebrowser/emacs.nix :noweb yes
{ inputs, ... }:
...
** Chat
#+begin_src nix-ts :tangle modules/chat.nix
{ inputs, ... }:
...
** Mutype
#+begin_src nix-ts :tangle modules/mutype.nix
{
...
** Gaming
Setting up your alsoftrc correctly makes Minecraft happy
#+begin_src nix-ts :tangle modules/gaming.nix
{
