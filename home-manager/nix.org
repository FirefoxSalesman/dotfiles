...
*** eshell
#+begin_src nix-ts :tangle modules/shells/eshell.nix
{
  flake.homeModules.shellConfig =
    { pkgs, ... }:
    {
...
Citre was borrowed from jweigley's config
...
*** Aphelia
#+begin_src nix-ts :tangle modules/development/apheleia.nix
{ ... }:
...
*** Buffer Terminator
#+begin_src nix-ts :tangle modules/window-manager/buffer-terminator.nix :noweb yes
{ ... }:
...
#+NAME: Buffer Terminator Preface
#+begin_src emacs-lisp
...
*RAG tools:*
+ [X] https://github.com/positron-solutions/ragmacs should be used to provide an introspection preset
+ [ ] https://git.bajsicki.com/phil/gptel-got should probably be another preset, but I'm not sure how that would look right now.

*Agents:*
+ [X] Try gptel-agent at some point
+ [X] Put macher in NixEmacs. It shouldn't need too much modification
+ [X] Try macher-agent
