...
**** Framework
#+begin_src nix :tangle keybinds/default.nix
{ ... }:
...
**** Evil
#+begin_src nix :tangle keybinds/evil.nix
{ pkgs, config, lib, ... }:

let bindOption = key: desc: lib.mkOption {
      type = lib.types.str;
      default = key;
      description = "Binding for evil-${desc}.";
    };
    keybinds = config.programs.emacs.init.keybinds;
    keys = keybinds.evil.keys;
in
{
  options.programs.emacs.init.keybinds.evil = {
    enable = lib.mkEnableOption "Enables evil-mode";
    keys = {
      append = bindOption "a" "append";
      append-line = bindOption "S-${keys.append}" "append-line";
      change = bindOption "c" "change";
      change-line = bindOption "S-${keys.change}" "change-line";
      delete = bindOption "d" "delete";
      delete-line = bindOption "S-${keys.delete}" "delete-line";
      insert = bindOption "i" "insert";
      insert-line = bindOption "S-${keys.insert}" "insert-line";
      join = bindOption "J" "join";
      set-marker = bindOption "m" "set-marker";
      open-below = bindOption "o" "open-below";
      open-above = bindOption "S-${keys.open-below}" "open-above";
      paste-after = bindOption "p" "paste-after";
      paste-before = bindOption "S-${keys.paste-before}" "paste-before" ;
      record-macro = bindOption "q" "record-macro";
      execute-last-recorded-macro = bindOption "S-${keys.record-macro}" "execute-last-recorded-macro";
      replace = bindOption "r" "replace";
      enter-replace-state = bindOption "S-${keys.replace}" "enter-replace-state";
      substitute = bindOption "s" "substitute";
      change-whole-ilne bindOption "S-${keys.substitute}" "change-whole-ilne";
      delete-char = bindOption "x" "delete-char";
      delete-backward-char = bindOption "S-${keys.insert}" "delete-backward-char";
      yank = bindOption "y" "yank";
      yank-line = bindOption "S-${keys.yank}" "yank-line";
      ex-repeat-substitute = "&" "ex-repeat-substitute";
      g-prefix = lib.mkOption {
        type: lib.types.str;
        default = "g";
        description = "The key used for evil's g-prefix";
      };
      z-prefix = lib.mkOption {
        type: lib.types.str;
        default = "z";
        description = "The key used for evil's z-prefix";
      };
      repeat = bindOption "." "repeat";
      execute-macro = bindOption "@" "execute-macro";
      use-register = bindOption "\\" "use-register";
      invert-char = bindOption "~" "invert-char";
      indent = bindOption "=" "indent";
      shift-left = bindOption "<" "shift-left";
      shift-right = bindOption ">" "shift-right";
      undo = bindOption "u" "undo";
      redo = bindOption "C-r" "redo";
      backward-word-begin = bindOption "b" "backward-word-begin";
      backward-WORD-begin = bindOption "S-${keys.backward-word-begin}" "backward-WORD-begin";
      forward-word-end = bindOption "e" "forward-word-end";
      forward-WORD-end = bindOption "S-${keys.forward-WORD-end}" "forward-WORD-end";
      find-char = bindOption "f" "find-char";
      find-char-backward = bindOption "S-${keys.find-char}" "find-char-backward";
      goto-line = bindOption "S-${keys.b-prefix}";
      backward-char = bindOption "h" "backward-char";
      window-top = bindOption "S-${keys.backward-char}" "window-top";
      next-line = bindOption "j" "next-line";
      previous-line = bindOption "k" "previous-line";
      forward-char = bindOption "l" "forward-char";
      lookup = bindOption "K" "lookup";
      window-bottom = bindOption "S-${keys.forward-char}" "window-bottom";
      window-middle = bindOption "M" "window-middle";
      search-next = bindOption "n" "search-next";
      search-previous = bindOption "S-${keys.search-next}" "search-previous";
      find-char-to = bindOption "t" "find-char-to";
      find-char-to-backward = bindOption "S-${keys.find-char-to}" "find-char-to-backward";
      forward-word-begin = bindOption "w" "forward-word-begin";
      forward-WORD-begin = bindOption "S-${keys.forward-word-begin}" "forward-WORD-begin";
      backward-paragraph = bindOption "{" "backward-paragraph";
      forward-paragraph = bindOption "}" "forward-paragraph";
      search-word-backward = bindOption "#" "search-word-backward";
      end-of-line = bindOption "$" "end-of-line";
      jump-item = bindOption "%" "jump-item";
      goto-mark = bindOption "`" "goto-mark";
      goto-mark-line = bindOption "'" "goto-mark-line";
    };
  };
}
#+end_src
