...
*** Svelte ts mode
#+begin_src nix :tangle packages/emacs/svelte-ts-mode.nix
{ trivialBuild, inputs } :
...
**** Swift
#+begin_src nix :tangle language-support/swift.nix
{ pkgs, config, lib, ... }:
...
**** Svelte
#+begin_src nix :tangle language-support/svelte.nix
{ pkgs, lib, config, ... }:
...
***** Evil-Surround
#+begin_src nix :tangle keybinds/evil-surround.nix
{ config, lib, ... }:
...
***** Electric Pair
#+begin_src nix :tangle keybinds/electric-pair.nix
{ config, lib, ... }:
...
***** Nerd Commenter
#+begin_src nix :tangle keybinds/evil-nerd-commenter.nix
{ config, lib, ... }:
...
***** Avy
#+begin_src nix :tangle keybinds/avy.nix
{ config, lib, ... }:
...
***** Ivy
+ [ ] Make it so that the mapping for find-file in leader keys is conditional
+ [ ] Move counsel-org-goto to org mode support
+ [ ] Counsel should integrate with helpful if helpful is present
#+begin_src nix :tangle completion/ivy.nix
{ config, lib, ... }:
...
***** Company
+ [ ] Move ledger stuff to ledger
#+begin_src nix :tangle completion/company.nix
{ config, lib, ... }:
