...
which-key-posframe = {
  ...
  config = ''
    <<Emacs Ricing Posframes Which Key Config>>
  '';
};
#+end_src

#+NAME: Emacs Ricing Posframes Which Key Config
#+begin_src emacs-lisp
...
**** dart
#+begin_src nix :tangle language-support/dart.nix
{ pkgs, config, lib, ... }:
...
***** leader key
#+begin_src nix :tangle keybinds/leader-key.nix
{ pkgs, lib, config, ... }:
...
#+begin_src nix :tangle keybinds/doom-escape.nix
{ pkgs, lib, config, ... }:
