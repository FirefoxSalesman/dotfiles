{
  flake.homeModules.aesthetics.programs.emacs.init.usePackage = {
          ...
          (let* ((shr-folding-mode 'none)
                 ...
                 (code
                  (with-temp-buffer
                    ...
                    (buffer-string)))
                 (lang
                  (or (shr-tag-pre-highlight-guess-language-attr pre)
                      (let ((sym (language-detection-string code)))
                        (and sym (symbol-name sym)))))
                 (mode
                  (and lang (shr-tag-pre-highlight--get-lang-mode lang))))
            (shr-ensure-newline)
            (shr-ensure-newline)
            (setq start (point))
            (insert
            ...
            (add-face-text-property
             start end '(:background "#1f2329" :extend t))
            (shr-ensure-newline)
            (insert "\n")))
      '';
      config = ''
      ...
      config = ''
        	  ...
        	  (defun shrface-render-advice (orig-fun &rest args)
        	    ...
        	    (let ((shrface-org nil)
        	          ...
        	          (shr-indentation 0)
        	          (shr-external-rendering-functions
        	           shrface-general-rendering-functions)
        	  ...
        	  (defun shrface-nov-render-html ()
        	    ...
        	    (let
        	        ((shrface-org nil)
        	         ...
        	         (tab-width 8)
        	         (shr-external-rendering-functions
        	          shrface-nov-rendering-functions)
        	         (shrface-href-versatile t)
        	  ...
        	  (with-eval-after-load 'eww
        	    (general-advice-add
        	     'eww-display-html
        	     :around #'shrface-render-advice))
        	  (with-eval-after-load 'elfeed
        	    (general-advice-add
        	     'elfeed-insert-html
        	     :around #'shrface-elfeed-advice))
        	  (with-eval-after-load (gsetq
        	                         nov-render-html-function
        	                         'shrface-nov-render-html))
        	  ...
        	  (add-to-list
        	   ...
        	   `((shrface-mode)
        	     ...
        	     :open-rec nil))
        	'';
      generalOneConfig.shr-map."RET" =
        "`,(cmd! (if (major-mode? 'eww-mode) (eww-follow-link) (shr-browse-url)))";
