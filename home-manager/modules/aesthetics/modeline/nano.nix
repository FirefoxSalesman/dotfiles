...

{
  ...
  flake.homeModules.aesthetics.programs.emacs.init.usePackage.doom-nano-modeline = {
    enable = true;
    afterCall = [ "after-init-hook" ];
    setopt.mode-line-format = false;
    config = ''
      (defun doom-nano-modeline--render
          (left right &optional hide-evil-mode)
             ...
             (pleft
              (concat
               (propertize " "
                           ...
                           evilstate-face
                           'display
                           `(raise ,doom-nano-modeline-top-padding))
      
               ;; Evil state.
               (when evilstate
                 (concat
                  (propertize evilstate 'face evilstate-face)
                  (propertize " " 'face modeline-face)))
      
               ;; Macro recording indicator.
               (when macrostring
                 (concat
                  (propertize macrostring 'face macro-face)
                  (propertize " " 'face modeline-face)))
      ...
      (defun doom-nano-modeline-buffer-name-vc-and-major-mode ()
        ...
        (let* ((buffer-name
                (cond
                 ...
                 ((and (buffer-narrowed-p) (buffer-base-buffer))
          ...
          `((,(concat buffer-modified buffer-name) . nil)
            ...
            (,(if vc-branch-name
                ...
                "")
             . nil)
            (,(if (or (major-mode? 'nix-ts-mode)
                      (major-mode? 'bibtex-ts-mode))
      ...
      (defun doom-nano-modeline--special-mode-p ()
        ...
        (or (derived-mode-p 'special-mode)
            (and (major-mode? 'exwm-mode) (not qutebrowser-exwm-mode))))
