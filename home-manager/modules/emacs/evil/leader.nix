  ...
  flake.homeModules.emacs = { pkgs, lib, ... }: {
    programs.emacs.init = {
      ...
      usePackage.emacs.generalOneConfig.global-leader = {
        "l" =
          ''`("Compile" . ,(cmd! (if (project-current) (project-compile) (compile (read-string "Compile command: " "make -k")))))'';
        "L" = ''`("Recompile" . ,(cmd! (if (project-current) (project-recompile) (recompile))))'';
        "u" =
          ''`("Mount USB" . ,(cmd! (start-process-shell-command "udisksmenu" nil "${lib.getExe pkgs.udisksmenu}")))'';
      };
    };
  };
}
